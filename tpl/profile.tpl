...
<div id="registrationbox">
	<form class="register" method="post" ajaxAction="{ProfileActions::Update}" id="frmRegister" action="{$smarty.server.SCRIPT_NAME}">
		<div class="registrationHeader"><h3>{translate key=Login}</h3></div>
		<p>
		...
		<div class="registrationHeader"><h3>{translate key=Profile}</h3></div>
		<p>
		...
		<div class="registrationHeader"><h3>{translate key=AdditionalInformation}</h3></div>
		<p>
...
{** TODO JS logic should be added  **}

		{if $Attributes|count > 0}
			<div class="registrationHeader" hidden><h3>{translate key=AdditionalAttributes}</h3></div>
			{foreach from=$Attributes item=attribute}
				{if $attribute->Id() eq -1 } {** Katılım değiştirme kapatıldı**}
