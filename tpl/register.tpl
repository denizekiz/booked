...
<div id="registrationbox">
	<form class="register" method="post" ajaxAction="{RegisterActions::Register}" id="frmRegister"
		...
		<p>
			<label class="reg" hidden>{translate key="DefaultPage"}<br/>
		...
		<p>
			<label class="reg" hidden>{translate key="Timezone"}<br/>
		...
		<p >
			<label class="reg" hidden>{translate key="Organization"}<br/>
		...
		{** TODO commented these attribute will be hidden in JS. **}
      {if $Attributes|count > 0}
		{* <div class="registrationHeader"><h3>{translate key=AdditionalAttributes}</h3></div> *}
		{foreach from=$Attributes item=attribute}
			{if $attribute->Id() eq 7 || $attribute->Id() eq 15 || $attribute->Id() eq 17}
			{if $attribute->Id() eq 15}
				<p>  Uyarı: Eğitim tipinizi seçip kayıt yaptırdıktan sonra bir daha değiştiremezsiniz. Eğitim tiplernin fiyatları hakkında bilgi almak için lütfen tıklayın. </p>

			{/if}
				{if $attribute->Id() eq 7}

				<p>  Uyarı: Yelken klübünüzü seçip kayıt yaptırdıktan sonra bir daha değiştiremezsiniz. Klüplere özel fırsatlar için tıklayınız. </p>
	...
	{/if}
		...
		<p>Kaydınız  1 Yıldız öğrencisi olarak alınacaktır. Eğer başka bir eğitime kaydolmak istiyorsanız,  kaydınız yapıldıktan sonra ve ödeme yapmadan önce diğer eğitimlere kayıt olmak lütfen için eğitim koordinatörü ile iletişime geçiniz.  </p>

		<div id="list"><p><iframe src="../egitim_sartnamesi.txt" width=400 height=400 frameborder=0 >
				</iframe></p></div>
