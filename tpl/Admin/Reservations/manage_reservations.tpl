...
<table class="list" id="reservationTable">
	...
	<!-- TODO /// Rezervasyon katılımcı numarası çekilecek. Array galiba boş geliyor -->		<td> {count($Page->GetReservationParticipants($reservation->ReferenceNumber))}{**$ReservationService->LoadByReferenceNumber($reservation->ReferenceNumber,$UserSession)->Participants()**} </td>
			<td>  {foreach from=($Page->GetReservationParticipants($reservation->ReferenceNumber))  item=participants} <p> {$participants}</p> {/foreach}</td>
		</tr>
	{/foreach}
</table>
