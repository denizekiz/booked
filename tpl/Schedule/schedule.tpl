...
{function name=displayGeneralReserved}
	{if $Slot->IsPending() && not $Slot->IsOwnedBy($MySession) && $IsAdmin }
	...
	<td {$spantype|default:'col'}span="{$Slot->PeriodSpan()}" class="reserved {$class} {$OwnershipClass} clickres slot"
		resid="{$Slot->Id()}" {$color}
		id="{$Slot->Id()}|{$Slot->Date()->Format('Ymd')}">{$Slot->Label($SlotLabelFactory)|escapequotes}  {if $IsAdmin} <p>-- Katılımcılar--</p> {foreach from=$Slot->getParticipantsNames() item=participant}<p> {$participant} </p> {/foreach} <p>-- Bekleyenler --</p> {foreach from=$Slot->getInviteeNames() item=invitee} <p> {$invitee} </p> {/foreach} {/if} <p> Kalan yer: {math equation="x - y" x=$Slot->GetMaxParticipants()  y=count($Slot->getParticipantsNames())} </p> </td>
{/function}
