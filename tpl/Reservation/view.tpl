	...
	<div id="reservationFormDiv">
				...
				<li style="padding-top:15px;">
					...
					{if $AllowParticipantsToJoin && !$IAmParticipating && !$IAmInvited}
						...
						<li id="joinReservation">
							Rezervasyona Katıl?
							{if $IsRecurring}
							{if $UserAttributes['saat'] > 6 ||  $UserAttributes['saat'] == 0}	<button value="{InvitationAction::JoinAll}" id="btnJoinSeries" class="button participationAction">{html_image src="ticket-plus.png"} {translate key=AllInstances}</button>
							{else}	<button value="{InvitationAction::Join}" id="btnJoinInstance" class="button participationAction">{html_image src="ticket-plus.png"} {translate key=ThisInstance}</button> {/if}
							{else}
		...
		<div style="float:right;">
			...
			<button type="button" class="button" onclick="window.location='{$ReturnUrl}'">
				<img src="img/tick.png"/> <!-- it was img/slash.png-->
				{translate key='Continue'} <!--it was close-->
			</button>
