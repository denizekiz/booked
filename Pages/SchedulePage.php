...
//ADded by deniz
	private function IsMyTrainingType(IReservationSlot $slot)
	{
		$mySession = ServiceLocator::GetServer()
			->GetUserSession();
		return $slot->IsSameTrainingType($mySession);
	}

	private function IsMyTrainingLevel(IReservationSlot $slot)
	{
		$mySession = ServiceLocator::GetServer()
			->GetUserSession();
		return $slot->IsSameTrainingLevel($mySession);
	}

	private function getParticipantNames(IReservationSlot $slot)
	{

		return $slot->getParticipantsNames();
	}

private function IsFull(IReservationSlot $slot)
{
	// DENİZ
//handled
    $maxparticipants=$slot->GetMaxParticipants();
	return  $slot->ParticipantNumber()>$maxparticipants-1;

}
private function AmIprivate ()
{
	$mySession = ServiceLocator::GetServer()
		->GetUserSession();
	$userrepo=new UserRepository();
	$user=$userrepo->LoadByPublicId($mySession->PublicId);
	$type=$user->GetAttributeValue(15);
	return strcmp("Özel",$type)==0;
	...
	private function HaveCredit()
	{
		$mySession = ServiceLocator::GetServer()
			->GetUserSession();
		$userrepo=new UserRepository();
		$user=$userrepo->LoadByPublicId($mySession->PublicId);
		$credi=$user->GetAttributeValue(8);
		return $credi>0;
	}
