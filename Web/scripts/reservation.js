function Reservation(opts)
{
	...
	var elements = {
		...
		// Credit attribute default value
		customAttributeCredit: $('.customAttribute input'),
		customAttributeTrainingLevel: $('select.customAttribute:first'),
		customAttributeTrainingType: $('select.customAttribute:last')
	};
	...
	Reservation.prototype.init = function (ownerId)
	{
		...
		// Credit custom attribute default value.

		elements.customAttributeCredit.val("2");
		if(!options.adminStatus)
		{
		elements.customAttributeTrainingLevel.val(options.userAttributes.trainingLevel);
		elements.customAttributeTrainingType.val(options.userAttributes.trainingType);}
